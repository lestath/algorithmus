...
public class For extends Polygon implements GraphicsBlockInterface{
	private  ForBlock block;
...
//	private Text notext;
	...
	public For(ForBlock blo){
		...
		//	this.block.getPosition().getY()+this.block.getSize().getHeight()/2,
			...
			this.block.getPosition().getX()+0.5*this.block.getSize().getWidth()+0.5*this.block.getSize().getWidth(),
			this.block.getPosition().getY()+this.block.getSize().getHeight(),
			this.block.getPosition().getX()+this.block.getSize().getWidth()+this.block.getSize().getWidth()*0.25,
			this.block.getPosition().getY()+this.block.getSize().getHeight()/2,
			this.block.getPosition().getX()+0.5*this.block.getSize().getWidth()+0.5*this.block.getSize().getWidth(),
	...
	public ForBlock getBlock() {
		return block;
	}
	public void setBlock(ForBlock block) {
		this.block = block;
	}
	...
	private void blockFieldRelocation(){
		this.blockfield.relocate(
				this.block.getPosition().getX()+0.1*this.block.getSize().getWidth(),
